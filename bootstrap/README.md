...
- **GitHub Release 预编译（~/.local/bin）**：starship、atuin、eza、sheldon、dust、fastfetch、lazygit、duf、doggo、gh、superfile、yazi、neovim

> 所有 GitHub 下载支持 `GH_PROXY` 代理前缀（大陆访问 GitHub 慢/超时可用）：
> `export GH_PROXY=https://ghfast.top/` 后重跑（备选：`gh-proxy.com`、`ghproxy.net`）。
...
5. **无需 Rust 工具链**：Rust 系工具（eza/sheldon/dust/starship/atuin）均用 GitHub Release 预编译二进制，不装 rustup/cargo（避免大陆下载 static.rust-lang.org 缓慢超时）。
