# home/linux/gui/fcitx5.nix — Fcitx5 Linux 专属（IM 注册 + 环境变量）
#
# 通用配置（rime 数据 / 主题 / fcitx5 主配置）在 home/base/gui/fcitx5.nix，
# 本文件仅负责 Linux 特有的输入法前端注册与 session 环境变量。
{ config, lib, pkgs, ... }:
let
  cfg = config.modules-home-linux-gui-fcitx5;
in
{
  options.modules-home-linux-gui-fcitx5 = {
