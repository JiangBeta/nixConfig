# home/base/fcitx5.nix — Fcitx5 跨平台通用配置
#
# Rime 雾凇拼音数据 + macos12-dark 主题 + fcitx5 主配置/输入法/经典 UI 配置。
# Linux 专属（i18n.inputMethod 注册 + session 环境变量）见 home/linux/Desktop/fcitx5.nix。
#
# 策略：将所有 rime-ice 文件 + 自定义配置打包到一个 derivation，
# 整个 symlink 到 ~/.local/share/fcitx5/rime/，与 Arch 安装效果一致。
{ config, lib, pkgs, ... }:
let
  cfg = config.modules-home-base-fcitx5;
...
in
{
  options.modules-home-base-fcitx5 = {
